package cz.hcasc.dagmar.storage
...
/**
 * Secure storage for DAGMAR Android app.
 *
 * Stores ONLY:
 * - instance_id
 * - instance_token (Bearer token for attendance API)
 * - device_fingerprint (UUID)
 * - device_name (uživatelsky zadaný název pro admin)
 * - display_name (vrací backend po claim-token)
 *
 * Does NOT store attendance data or any offline history.
 */
class SecureStore(context: Context) {
    ...
    fun setDeviceFingerprint(value: String) {
        prefs.edit().putString(KEY_DEVICE_FINGERPRINT, value).apply()
    }

    fun getInstanceId(): String? = prefs.getString(KEY_INSTANCE_ID, null)

    fun setInstanceId(value: String) {
        prefs.edit().putString(KEY_INSTANCE_ID, value).apply()
    }

    fun getInstanceToken(): String? = prefs.getString(KEY_INSTANCE_TOKEN, null)

    fun setInstanceToken(value: String) {
        prefs.edit().putString(KEY_INSTANCE_TOKEN, value).apply()
    }
    ...
    fun clearInstanceToken() {
        prefs.edit().remove(KEY_INSTANCE_TOKEN).apply()
    }

    fun clearInstanceId() {
        prefs.edit().remove(KEY_INSTANCE_ID).apply()
    }
    ...
    companion object {
        private const val PREFS_NAME = "dagmar_secure_store"
